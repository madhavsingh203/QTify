...
const Carousel = ({ data, carouselKey }) => {
  ...
  return (
    <div className="relative">
      ...
      {/* Next and Previous buttons with conditional rendering */}
      {window.innerWidth > 767 && (
        <div className={`arrow-right-${carouselKey} absolute -right-2 top-1/3 z-10 bg-white rounded-full`}><NextIcon /></div>
      )}
      {window.innerWidth > 767 && (
        <div className={`arrow-left-${carouselKey} absolute -left-3 top-1/3 z-10 bg-white rounded-full`}><PrevIcon /></div>
      )}
    </div>
  );
};
